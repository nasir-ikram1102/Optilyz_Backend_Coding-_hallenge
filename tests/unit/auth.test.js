...
describe('Auth routes', () => {
  ...
  describe('POST /auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      await tokenService.saveToken(refreshToken, userOne, expires, tokenTypes.REFRESH);
      ...
      expect(dbRefreshTokenDoc).toMatchObject({ type: tokenTypes.REFRESH, user: userOne, blacklisted: false });
    ...
    test('should return 401 error if refresh token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne, expires, tokenTypes.REFRESH);
      await tokenService.saveToken(refreshToken, userOne, expires, tokenTypes.REFRESH, true);
